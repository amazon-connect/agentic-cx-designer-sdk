...
/**
 * Trailhead Assistant - build + deploy step.
 *
 * Follow-up to `npm run trailhead:create`. Builds the application, waits for
 * the build to finish, then deploys it (create-or-update deployment in place).
 *
 * Run with:  npm run trailhead:build-deploy
 */
...
(async () => {
  ...
  try {
    ...
    if (!app) {
      throw new Error(`Application "${APPLICATION.name}" not found. Run "npm run trailhead:create" first.`);
    }
    ...
    const build = await client.send(
      new sdk.CreateApplicationBuildCommand({
        ...
        description: 'Trailhead assistant build (via example scripts).',
      }),
    );
